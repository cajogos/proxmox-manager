...
export async function vmStop(vmid: number, opts: VMActionOptions): Promise<void> {
  ...
  if (checkDryRun(opts.dryRun, 'stop VM', String(vmid))) {
    console.log(dryRunMsg(`Would hard-stop VM ${vmid}.`));
    audit({ ...auditBase, timestamp: new Date().toISOString(), result: 'dry-run', error: null });
    return;
  }

  const confirmed = await confirmAction('Hard-stop VM', String(vmid), opts.yes);
  if (!confirmed) {
    audit({ ...auditBase, timestamp: new Date().toISOString(), result: 'cancelled', error: null });
    return;
  }

  const spinner = startSpinner(`Stopping VM ${vmid}…`);
  const result = await vmActionService(config, vmid, 'stop', { profile: opts.profile, node: opts.node });
  spinner.stop();
  ...
  console.log(successMsg(`VM ${vmid} stopped.`));

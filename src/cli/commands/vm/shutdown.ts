...
export async function vmShutdown(vmid: number, opts: VMActionOptions): Promise<void> {
  ...
  if (checkDryRun(opts.dryRun, 'shutdown VM', String(vmid))) {
    console.log(dryRunMsg(`Would gracefully shut down VM ${vmid}.`));
    audit({ ...auditBase, timestamp: new Date().toISOString(), result: 'dry-run', error: null });
    return;
  }

  const confirmed = await confirmAction('Gracefully shut down VM', String(vmid), opts.yes);
  if (!confirmed) {
    audit({ ...auditBase, timestamp: new Date().toISOString(), result: 'cancelled', error: null });
    return;
  }

  const spinner = startSpinner(`Shutting down VM ${vmid}…`);
  const result = await vmActionService(config, vmid, 'shutdown', { profile: opts.profile, node: opts.node });
  spinner.stop();
  ...
  console.log(successMsg(`VM ${vmid} shutdown initiated.`));

...
export async function lxcReboot(ctid: number, opts: LXCActionOptions): Promise<void> {
  ...
  if (checkDryRun(opts.dryRun, 'reboot container', String(ctid))) {
    console.log(dryRunMsg(`Would reboot container ${ctid}.`));
    audit({ ...auditBase, timestamp: new Date().toISOString(), result: 'dry-run', error: null });
    return;
  }

  const confirmed = await confirmAction('Reboot container', String(ctid), opts.yes);
  if (!confirmed) {
    audit({ ...auditBase, timestamp: new Date().toISOString(), result: 'cancelled', error: null });
    return;
  }

  const spinner = startSpinner(`Rebooting container ${ctid}…`);
  const result = await lxcActionService(config, ctid, 'reboot', { profile: opts.profile, node: opts.node });
  spinner.stop();
  ...
  console.log(successMsg(`Container ${ctid} reboot initiated.`));
